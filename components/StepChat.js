...
import {
  sendChatMessage, generateShorts,
  toAbsoluteUrl, bustCache, applyEffect,
} from '../api';
...
export function StepChat({ jobData, onDone, planCtx, onPaywall, onEffectApplied }) {
  ...
  const [messages, setMessages] = useState([{ id: '0', role: 'ai', text: `Video yüklendi (${jobData.duration}s). Mic'e bas, konuş, tekrar bas.` }]);
  ...
  async function sendMsg(text) {
    ...
    setInput(''); addMsg('user', msg); setLoading(true); setConfirmData(null);
    // Auto-detect effect intent from message
    ...
    try {
      ...
      const data = await sendChatMessage({ message: msg, history, jobId: jobData.jobId, language: voiceLang });
      ...
      // If confirm box appeared, prompt user to confirm by voice
      if (data.ready_to_edit && data.edit_params) {
      ...
      const e = 'Bir hata oluştu, tekrar dener misin?';
      addMsg('ai', e);
      speak(e, () => handleStartMic());
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleEffectSelect(category, intensity) {
    ...
    try {
      ...
      const data = await applyEffect(jobData.jobId, { category, intensity }, plan);
  ...
  async function doGenerateShorts() {
    ...
    try {
      const plan = planCtx?.isPro ? 'pro' : 'free';
      const data = await generateShorts(jobData.jobId, { topN: 5 }, plan);
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            <View style={s.voiceBar}>
              {speaking
                ...
                : (<><Text style={[s.voiceDot, { color: C.accent }]}>🎤</Text>
                    ...
                    <Text style={{ color: C.muted, fontSize: 11 }}>Mic bas → gönder</Text></>)}
            </View>
          )}
          ...
          <View style={s.inputRow}>
            {micAvail && !editLoading && (
              <TouchableOpacity
                ...
                onPress={() => listening ? stopMicAndSend() : handleStartMic()}
            ...
            <TextInput
              style={s.chatInput}
              placeholder={listening ? (interim || 'Dinliyorum...') : 'Yaz veya mic ile konuş'}
      ...
      {tab === 'shorts' && (
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          ...
          {!!shortsError && <Text style={[s.errTxt, { marginTop: 12 }]}>{shortsError}</Text>}
          {shorts.map(clip => (
